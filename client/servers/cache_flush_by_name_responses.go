// Code generated by go-swagger; DO NOT EDIT.

package servers

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"models"
)

// CacheFlushByNameReader is a Reader for the CacheFlushByName structure.
type CacheFlushByNameReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *CacheFlushByNameReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewCacheFlushByNameOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewCacheFlushByNameOK creates a CacheFlushByNameOK with default headers values
func NewCacheFlushByNameOK() *CacheFlushByNameOK {
	return &CacheFlushByNameOK{}
}

/*CacheFlushByNameOK handles this case with default header values.

Flush successful
*/
type CacheFlushByNameOK struct {
	Payload *models.CacheFlushResult
}

func (o *CacheFlushByNameOK) Error() string {
	return fmt.Sprintf("[PUT /servers/{server_id}/cache/flush][%d] cacheFlushByNameOK  %+v", 200, o.Payload)
}

func (o *CacheFlushByNameOK) GetPayload() *models.CacheFlushResult {
	return o.Payload
}

func (o *CacheFlushByNameOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.CacheFlushResult)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
